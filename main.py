...
def get_5_from_2(a, Rs, p3):
    ...
    I0 = (alpha * (p2[1] - p1[1]) + beta * (p3[1] - p1[1])) / (Np * (alpha * (
         np.exp((p1[0] + p1[1] * Rs * (Ns / Np)) / (a * Vt)) -
         np.exp((p2[0] + p2[1] * Rs * (Ns / Np)) / (a * Vt))) + beta * (
         np.exp((p1[0] + p1[1] * Rs * (Ns / Np)) / (a * Vt)) -
         np.exp((p3[0] + p3[1] * Rs * (Ns / Np)) / (a * Vt)))))

    Rp = ((p1[0] - p2[0]) * (Np / Ns) + Rs * (p1[1] - p2[1])) / \
         (p2[1] - p1[1] - I0 * Np * (np.exp((p1[0] + p1[1] * Rs * (Ns / Np)) / (a * Vt)) -
                                     np.exp((p2[0] + p2[1] * Rs * (Ns / Np)) / (a * Vt))))

    Ipv = (I0 * Np * (np.exp((p1[0] + p1[1] * Rs * (Ns / Np)) / (a * Vt)) - 1) + (
...
def plot_from_2(a, Rs, p3):
    ...
    rp, rs, a, i0, ipv = get_5_from_2(a, Rs, p3)
    i = pvlib.pvsystem.i_from_v(rp, rs, a * Vt, v, i0, ipv, method="newton")
    plt.plot(v, i)
    ...
    while gen <= GENMAX:
        ...
        for i in range(NP):
            ...
            if f < fitness[i]:
                ...
                v = np.linspace(0, 25, 100)
                rp, rs, a, i0, ipv = get_5_from_2(a, Rs, p3)
                ical = pvlib.pvsystem.i_from_v(rp, rs, a * Vt, v, i0, ipv, method="newton")
                plt.plot(v, ical)
                plt.xlabel("Voltage V(V)")
                plt.ylabel("Current I(A)")
                plt.grid()
